...
<section class="flex flex-col gap-4">
  <div class="flex gap-4 items-center">
    <img src={profile.avatar} alt={`${profile.handle} profile picture`} class="size-16 rounded-sm" />
  ...
  <section class="w-full flex gap-4 h-fit">
    <IconDrawer>
      {#snippet trigger()}
        <Button.Root class="flex items-center bg-pink-500 px-4 py-2 rounded-sm w-full gap-4 text-sm hover:bg-pink-600/95 active:scale-95 active:transition-all duration-150">
      ...
      {#snippet content()}
        <div class="flex flex-col gap-2">
          <a
            ...
            target="_blank" 
            class="flex gap-4 items-center hover:bg-white/10 px-4 py-2 rounded-sm transition-all duration-150"
          >
